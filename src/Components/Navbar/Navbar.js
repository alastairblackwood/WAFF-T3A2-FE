...
const Navbar = () => {
  ...
  return (
    <NavbarContainer className="navbar navbar-light text-uppercase">
      <NavA to="/" className="navtitle" href="/">
        WES ANDERSON FILM FESTIVAL
      </NavA>
      {/* toggler */}

      <NavCollapse>
        <NavLi to="/films" aria-current="page">
          Films
        </NavLi>
        <NavLi to="/about" aria-current="page">
          About
        </NavLi>
        <NavLi to="/schedule" aria-current="page">
          Schedule
        </NavLi>
        {auth.isLoggedIn ? (
          <>
            <NavLi to="/dashboard" aria-current="page">
              Dashboard
            </NavLi>
            <NavButton onClick={handleLogout}>LOGOUT</NavButton>
          </>
        ) : (
          <NavLi aria-current="page" to="/login">
            Login
          </NavLi>
        )}
      </NavCollapse>
